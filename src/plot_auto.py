...
points = [[0.0]*3 for i in range(272)]
regime = [0]*272
...
supv = [points[i] for i in range(272) if regime[i] == 1]
vir = [points[i] for i in range(272) if regime[i] == 2]
subv = [points[i] for i in range(272) if regime[i] == 3]	

fig = plt.figure()
ax = fig.add_subplot(111, projection='3d')
for c, m, l in [('y', 'o', supv), ('b', '^', vir), ('r', 'o', subv)]:
	points = [[0.0]*272 for i in range(3)]
	for i in range(3):
